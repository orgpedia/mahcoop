...
def main():
    ...
    start_num = max((get_doc_num(d["name"]) for d in documents_dict.values()), default=0)
    ...
    if documents_file.exists():
        documents_file_dict = yaml.load(documents_file.read_text(), Loader=yaml.FullLoader)
